...
namespace ChurchRegister.ApiService.Tests.Services;
...
public class ChurchMemberServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task GetDashboardStatisticsAsync_ReturnsCorrectStatistics()
    {
        // Arrange
        ...
        _context.ChurchMembers.AddRange(
            ...
            new ChurchMember
            {
                ...
                ChurchMemberStatusId = 1,
                MemberSince = now.AddDays(-15), // New member this month
                CreatedBy = "test",
                CreatedDateTime = now
            }
        );
        ...
        _context.EventAttendances.AddRange(
            new EventAttendance { EventId = 1, Date = now.AddDays(-7), Attendance = 50, CreatedBy = "test", CreatedDateTime = now },
            new EventAttendance { EventId = 1, Date = now.AddDays(-14), Attendance = 60, CreatedBy = "test", CreatedDateTime = now }
        );
        ...
        result.NewMembersThisMonth.Should().Be(1);
    }

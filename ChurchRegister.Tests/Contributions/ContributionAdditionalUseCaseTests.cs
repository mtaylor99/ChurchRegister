...
public class ContributionAdditionalUseCaseTests
{
    ...
    {
        // Arrange
        var expected = new AssignTransactionResponse(true, "Assigned", 1);
        _mockUnmatchedService
            .Setup(s => s.AssignTransactionToMemberAsync(10, 5, "admin", It.IsAny<CancellationToken>()))
            .ReturnsAsync(expected);

        var useCase = new AssignTransactionUseCase(_mockUnmatchedService.Object);
        ...
        _mockUnmatchedService.Verify(
            s => s.AssignTransactionToMemberAsync(10, 5, "admin", It.IsAny<CancellationToken>()),
            Times.Once);
    }

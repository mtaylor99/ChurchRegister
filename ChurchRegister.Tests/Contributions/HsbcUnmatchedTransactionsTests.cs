...
public class HsbcUnmatchedTransactionsTests : IDisposable
{
    ...
    public async Task AC001_AssignTransaction_HappyPath_UpdatesMemberAndCreatesContribution()
    {
        ...
        // Act
        var result = await _service.AssignTransactionToMemberAsync(tx.Id, member.Id, TestUser, default);
    ...
    public async Task AC002_AfterAssignment_SameReferencePendingTransactions_GetProcessed()
    {
        ...
        var result = await _service.AssignTransactionToMemberAsync(tx1.Id, member.Id, TestUser, default);
    ...
    public async Task AC004_AssignConflict_WhenReferenceAlreadyBelongsToOtherMember_ThrowsInvalidOperation()
    {
        ...
        await _service.Invoking(s => s.AssignTransactionToMemberAsync(tx.Id, memberB.Id, TestUser, default))
    ...
    [Fact]
    public async Task AC007_AssignTransaction_WhenTransactionNotFound_ThrowsArgumentException_NoSideEffects()
    {
        // Arrange
        var member = AddMember("Test", "User");

        // Act & Assert
        await _service.Invoking(s => s.AssignTransactionToMemberAsync(9999, member.Id, TestUser, default))
    ...
    public async Task AC009_AssignTransaction_SetsCreatedByToAssignedByUser()
    {
        ...
        // Act
        await _service.AssignTransactionToMemberAsync(tx.Id, member.Id, assignedBy, default);

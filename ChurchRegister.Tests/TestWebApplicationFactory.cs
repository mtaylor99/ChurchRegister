...
public class TestWebApplicationFactory<TProgram> : WebApplicationFactory<TProgram> where TProgram : class
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        // Configure test services — runs AFTER all other registrations
        builder.ConfigureTestServices(services =>
        {
            // Program.cs skips DbContext registration in the Testing environment,
            // so we only need to add the in-memory database here (no removal needed).
    ...
    private string GenerateTestToken(string userId, string email, string[] roles)
    {
        ...
        // Use the same defaults as Program.cs JWT configuration so Bearer validation passes
        var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes("ChurchRegister-Super-Secret-Key-For-Development-Only-2024!"));
    ...
    protected override void Dispose(bool disposing)
    {
        if (disposing)
        {
            // Clean up the in-memory database (guard against already-disposed provider)
            try
            {
                using var scope = Services.CreateScope();
                var context = scope.ServiceProvider.GetRequiredService<ChurchRegisterWebContext>();
            ...
            catch (ObjectDisposedException) { /* factory already disposed, nothing to clean up */ }
        }

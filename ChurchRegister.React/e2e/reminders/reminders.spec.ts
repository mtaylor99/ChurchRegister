 ...
 * - Export Reminders button is present on the Reminders tab
 *
...
test.describe('Reminders', () => {
  ...
  test('Export Reminders button is visible on the Reminders tab', async ({
    page,
  }) => {
    await expect(
      page.getByRole('button', { name: /Export Reminders/i })
    ).toBeVisible({ timeout: 10_000 });
  });

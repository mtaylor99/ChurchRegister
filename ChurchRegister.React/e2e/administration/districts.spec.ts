 ...
 * - Export Districts button is present
 *
...
test.describe('Administration - Districts', () => {
  ...
  test('Export Districts button is visible', async ({ page }) => {
    await expect(
      page.getByRole('button', { name: /Export.*District|District.*Export/i })
  ...
  test('clicking a districts row shows action buttons or opens a dialog', async ({
    ...
    if (rowCount > 0) {
      ...
      const moreActionsBtn = rows.first().getByRole('menuitem', { name: /More actions/i })
        .or(rows.first().getByRole('button', { name: /More actions|Actions/i }));

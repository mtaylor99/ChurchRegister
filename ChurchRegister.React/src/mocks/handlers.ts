...
export const contributionsHandlers = [
  ...
  // Assign unmatched HSBC transaction to member(s)
  http.post(
    `${API_BASE_URL}/api/financial/hsbc-transactions/:id/assign`,
    async ({ request, params }) => {
      const { id } = params;
      const body = (await request.json()) as {
        churchMemberId: number;
        secondaryChurchMemberId?: number;
      };

      // Simulate duplicate member selection (400)
      if (
        body.secondaryChurchMemberId &&
        body.churchMemberId === body.secondaryChurchMemberId
      ) {
        return HttpResponse.json(
          { message: 'Primary and secondary members must be different' },
          { status: 400 }
        );
      }

      // Simulate conflicting bank reference (409)
      if (body.churchMemberId === 999 || body.secondaryChurchMemberId === 999) {
        return HttpResponse.json(
          {
            message:
      ...
      // Simulate shared reference assignment (2 members)
      if (body.secondaryChurchMemberId) {
        return HttpResponse.json({
          success: true,
          message: `Transaction ${id} assigned to 2 members`,
      ...
      // Simulate single member assignment
      return HttpResponse.json({
        success: true,
        message: `Transaction ${id} assigned to member ${body.churchMemberId}`,
        reProcessedMatchedCount: 0,

...
export interface GetUnmatchedTransactionsResponse {
  totalCount: number;
  items: UnmatchedTransactionDto[];
}

export interface AssignTransactionRequest {
  churchMemberId: number;
  secondaryChurchMemberId?: number; // For shared references
}

export interface AssignTransactionResponse {
  success: boolean;
  message: string;
  reProcessedMatchedCount: number;
  isSharedReference: boolean;
  contributionsCreated: number;
}

export interface SharedBankReferenceInfoDto {
  reference: string;
  members: SharedReferenceMemberDto[];
}

export interface SharedReferenceMemberDto {
  churchMemberId: number;
  fullName: string;
}

export interface ExcludeReferenceResponse {
  success: boolean;
  reference: string;
  message: string;
}

export interface ExcludedReferenceDto {
  id: number;
  reference: string;
  createdBy: string;
...
export interface GetExcludedReferencesResponse {
  items: ExcludedReferenceDto[];
}

...
const AssignTransactionDialog: React.FC<AssignDialogProps> = ({
  ...
  const { data: secondaryMembersResponse } = useQuery({
    queryKey: [
      'churchMembers',
      {
    ...
    queryFn: () =>
      churchMembersApi.getChurchMembers({
        page: 1,
        pageSize: 50,
        searchTerm: secondarySearchTerm || undefined,
        noBankReferenceFilter: true,
        sortBy: 'firstName',
        sortDirection: 'asc',
      }),
  ...
  const handleConfirm = () => {
    ...
    // Validate that primary and secondary members are different
    if (secondaryMember && selectedMember.id === secondaryMember.id) {
      setError('Primary and secondary members must be different');
      return;
    }

    assignMutation.mutate({
      churchMemberId: selectedMember.id,
      secondaryChurchMemberId: secondaryMember?.id,
    });
  };
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        ...
        <Autocomplete
          options={secondaryMembersResponse?.items || []}
          getOptionLabel={(option) => option.fullName || ''}
          value={secondaryMember}
          onChange={(_, newValue) => setSecondaryMember(newValue)}
          inputValue={secondarySearchTerm}
          onInputChange={(_, newInputValue) =>
            setSecondarySearchTerm(newInputValue)
          }
          disabled={assignMutation.isPending}
          renderInput={(params) => (
            <TextField
              {...params}
              label="Secondary Member (Optional)"
              placeholder="Type to search members..."
              helperText="Optional: Select a second member to split this contribution 50/50"
            />
          )}
          isOptionEqualToValue={(option, value) => option.id === value.id}
          noOptionsText={
            secondarySearchTerm ? 'No members found' : 'Start typing to search'
          }
        />

        {secondaryMember && selectedMember && (
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        ...
        <DialogContentText sx={{ mt: 1 }}>
          Transactions with this reference will no longer appear in the
          unmatched list and will not be automatically matched to members.
        </DialogContentText>
      </DialogContent>
      <DialogActions>
...
export const UnmatchedTransactionsPage: React.FC = () => {
  ...
  const [assignTarget, setAssignTarget] =
    useState<UnmatchedTransactionDto | null>(null);

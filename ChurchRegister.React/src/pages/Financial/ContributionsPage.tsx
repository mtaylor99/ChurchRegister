...
// ─── Tab helpers (matches TrainingCertificatesPage pattern) ──────────────────
...
function TabPanel({ children, value, index, ...other }: TabPanelProps) {
  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`contributions-tabpanel-${index}`}
      aria-labelledby={`contributions-tab-${index}`}
      {...other}
    >
...
function a11yProps(index: number) {
  return {
    id: `contributions-tab-${index}`,
    'aria-controls': `contributions-tabpanel-${index}`,
  };
}
 ...
 * - Tab 0: Contribution member grid with financial action buttons
 * - Tab 1: Unmatched HSBC transactions resolution
 *
...
export const ContributionsPage: React.FC = () => {
   ...
   * Refreshes the contribution grid data but leaves the modal open
   * so the user can read the upload summary before closing.
   */
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Header Right: Action Buttons — visible on Contributions tab only */}
        {tabValue === 0 && (
          <FinancialActionsHeader
      ...
      {/* Tab 0: Contribution Member Grid */}
      ...
      {/* Tab 1: Unmatched HSBC Transactions */}

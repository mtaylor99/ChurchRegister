...
export interface AttendanceAnalyticsPageHandle {
  openShare: (el: HTMLElement) => void;
}

export interface AttendanceAnalyticsPageProps {
  /** Whether the analytics tab is currently active */
  isActive?: boolean;
  /** Called when share button visibility should change (events available or not) */
  onShareVisibilityChange?: (visible: boolean) => void;
}
 ...
 */
export const AttendanceAnalyticsPage = forwardRef<
  AttendanceAnalyticsPageHandle,
  AttendanceAnalyticsPageProps
>(({ isActive = true, onShareVisibilityChange }, ref) => {
  ...
  const analyticsEventsAll = useMemo(
  ...
  const filteredEvents = useMemo(() => {
    if (!searchTerm.trim()) {
      return analyticsEventsAll;
    }

    return analyticsEventsAll.filter(
      (event) =>
        event.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        event.description?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [analyticsEventsAll, searchTerm]);
  ...
  const handleShareMenuOpen = (el: HTMLElement) => {
    setShareMenuAnchor(el);
  };
  ...
  useImperativeHandle(ref, () => ({
    openShare: (el: HTMLElement) => handleShareMenuOpen(el),
  }));

  useEffect(() => {
    onShareVisibilityChange?.(analyticsEventsAll.length > 0);
  }, [analyticsEventsAll.length, onShareVisibilityChange]);
  ...
  const analyticsEvents = analyticsEventsAll;
  ...
  return (
    ...
    >
      {/* Share Menu - triggered from parent header button */}
      <Menu

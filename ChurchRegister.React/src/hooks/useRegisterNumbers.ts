...
export function useGenerateRegisterNumbers() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      showSuccess(
        `Successfully generated ${data.totalMembersAssigned} Baptised Member, ${data.totalNonBaptisedMembersAssigned} Non-Baptised Member, and ${data.totalNonMembersAssigned} Non-Member numbers for ${data.year}`
      );
    },

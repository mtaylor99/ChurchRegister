...
import { riskAssessmentKeys } from '../utils/queryKeys';
 ...
 */
export const riskAssessmentQueryKeys = riskAssessmentKeys;
...
export const useRiskAssessments = (
  ...
  return useQuery({
    queryKey: riskAssessmentQueryKeys.list(
      categoryId,
      status,
      overdueOnly,
      title
    ...
    queryFn: () =>
      riskAssessmentsApi.getRiskAssessments(
        categoryId,
        status,
        overdueOnly,
        title
...
export const useCreateRiskAssessment = () => {
  ...
  return useMutation({
    ...
    onError: (error: unknown) => {
      showError(extractErrorMessage(error, 'Failed to create risk assessment'));
    },
  });
};
...
export const useUpdateRiskAssessment = () => {
  ...
  return useMutation({
    ...
    onError: (error: unknown) => {
      showError(extractErrorMessage(error, 'Failed to update risk assessment'));
    },
  });
};
...
export const useStartReview = () => {
  ...
  return useMutation({
    ...
    onError: (error: unknown) => {
      showError(extractErrorMessage(error, 'Failed to start review'));
    },
  });
};
...
export const useApproveRiskAssessment = () => {
  ...
  return useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (response.assessmentApproved) {
        const nextReviewDate = response.nextReviewDate
          ...
          : 'N/A';
        showSuccess(`Risk assessment approved! Next review: ${nextReviewDate}`);
      } else {
        showSuccess(
    ...
    onError: (error: unknown) => {
      showError(
        extractErrorMessage(error, 'Failed to approve risk assessment')
      );
    },
  });
};
...
export const useDeleteRiskCategory = () => {
  ...
  return useMutation({
    ...
    onError: (error: unknown) => {
      showError(extractErrorMessage(error, 'Failed to delete category'));
    },
  });
};

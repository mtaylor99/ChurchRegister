...
const MonthlyReportPackWidget = () => {
  ...
  const handleGenerateClick = async () => {
    ...
    const reports: Array<{
      ...
      {
        name: 'Pastoral Care',
        fetch: getPastoralCareReport,
        filename: 'Pastoral-Care.pdf',
      },
      { name: 'Training', fetch: getTrainingReport, filename: 'Training.pdf' },
      {
        name: 'Risk Assessments',
        fetch: getRiskAssessmentsReport,
        filename: 'Risk-Assessments.pdf',
      },
      {
        name: 'Reminders',
        fetch: getRemindersReport,
        filename: 'Reminders.pdf',
      },
    ];
    ...
    if (successCount === reports.length) {
      setSnackbarMessage(
        `All ${successCount} reports downloaded successfully.`
      );
      setSnackbarSeverity('success');
    } else if (successCount > 0) {
      setSnackbarMessage(
        `${successCount} of ${reports.length} reports downloaded. Failed: ${failedReports.join(', ')}`
      );
      setSnackbarSeverity('success');
    } else {
      setSnackbarMessage(
        'All reports failed to generate. Please check your permissions.'
      );
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Download monthly reports (Attendance Analytics, Pastoral Care,
            Training, Risk Assessments, Reminders)
          </Typography>
          <Button
            variant="outlined"
            color="primary"

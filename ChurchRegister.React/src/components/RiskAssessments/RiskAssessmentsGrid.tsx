...
export interface RiskAssessmentsGridProps {
...
export const RiskAssessmentsGrid = React.memo(function RiskAssessmentsGrid({
  riskAssessments,
  loading,
  categories,
  categoryId,
  ...
  const hasActiveFilters = useMemo(
    () => categoryId !== null || status !== '' || overdueOnly || title !== '',
    [categoryId, status, overdueOnly, title]
  );

  const handleActionClick = useCallback(
    (event: React.MouseEvent<HTMLElement>, assessment: RiskAssessment) => {
  ...
  const handleConfirmStartReview = useCallback(() => {
    if (selectedAssessment) {
      onStartReview(selectedAssessment);
      setSelectedAssessment(null);
    }
    setConfirmDialogOpen(false);
  }, [selectedAssessment, onStartReview]);
  ...
  const columns: GridColDef[] = useMemo(
    () => [
      ...
      },
      {
        ...
        renderCell: (params: GridRenderCellParams) => {
          const date = format(
            new Date(params.row.nextReviewDate),
            'dd MMM yyyy'
          );
          ...
          return (
            <Typography sx={{ color, fontWeight: 500 }}>{date}</Typography>
          );
        },
      },
      {
  ...
  return (
    <Box>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            This will clear all approvals and set the status to "Under Review".

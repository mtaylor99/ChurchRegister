...
export interface AddRiskAssessmentDrawerProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
export function AddRiskAssessmentDrawer({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(extractErrorMessage(err, 'Failed to create risk assessment'));
    }
  };
  ...
  return (
    <Drawer anchor="right" open={open} onClose={handleClose}>
      <Box sx={{ width: 600, p: 3 }}>
        ...
        <Stack spacing={3}>
          ...
          <FormControl fullWidth required>
            ...
            <Select
              ...
              onChange={(e) =>
                setReviewInterval(e.target.value as 1 | 2 | 3 | 5)
              }
              disabled={isSaving}
            >
        ...
        <Box sx={{ mt: 3 }}>
          <Alert severity="info">
            New risk assessments are created with status "Under Review" and will
            require the minimum number of approvals before being activated.
          </Alert>
        </Box>
      </Box>
    </Drawer>

...
export const UserGrid: React.FC<UserGridProps> = React.memo(({
  ...
  const resendInvitationMutation = useMutation({
    mutationFn: (userId: string) => administrationApi.resendInvitation(userId),
    onSuccess: (result) => {
      if (result.emailSent) {
    ...
    onError: (error) => {
      ...
      notification.showError('Failed to resend invitation. Please try again.');
      setConfirmation((prev) => ({ ...prev, loading: false }));
    },
  });
      ...
      },
      {
        ...
        renderCell: (params) => (
          <Typography variant="body2">
            {new Date(params.row.dateJoined).toLocaleDateString()}
          </Typography>
        ),
      },
      {
    ...
    <ErrorBoundary>
      <Box sx={{ width: '100%' }}>
          ...
          {actionMenu.user &&
            getAvailableActions(actionMenu.user.status).map((action) => {
              const icons = {
                ...
                [UserStatusAction.Activate]: <ActivateIcon fontSize="small" />,
        ...
        <ConfirmationModal
          ...
          title={`${confirmation.action ? getStatusActionLabel(confirmation.action) : 'Confirm'} User`}

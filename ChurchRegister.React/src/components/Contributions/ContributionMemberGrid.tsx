...
export const ContributionMemberGrid: React.FC<ContributionMemberGridProps> =
  React.memo(({ onViewContributions, initialQuery }) => {
    ...
    const [sortModel, setSortModel] = useState<GridSortModel>([
      {
        field: initialQuery?.sortBy || 'lastContributionDate',
    ...
    const searchQuery: ContributionGridQuery = useMemo(
      () => ({
        ...
        sortBy: sortModel[0]?.field || 'lastContributionDate',
    ...
    const columns: GridColDef<ContributionMemberDto>[] = useMemo(
        ...
        },
        {
          ...
          renderHeader: () => (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, overflow: 'visible' }}>
              <Box component="span" sx={{ fontWeight: 500, fontSize: '0.875rem', whiteSpace: 'nowrap' }}>
                Contribution
              </Box>
              <Switch
                size="small"
                ...
                inputProps={{ 'aria-label': 'Show contribution amounts' }}
      ...
      [handleActionMenuOpen, selectedYear, showContributions, setShowContributions]
    );

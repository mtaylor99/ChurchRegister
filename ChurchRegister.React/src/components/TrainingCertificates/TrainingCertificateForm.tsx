...
> = ({ certificate, mode, onSuccess, onCancel }) => {
  ...
  const { data: types = [] } = useQuery<TrainingCertificateTypeDto[]>({
    ...
    queryFn: () =>
      trainingCertificatesApi.getTrainingCertificateTypes('Active'),
  });
  ...
  const { data: membersResponse } = useQuery({
    queryKey: [
      'churchMembers',
      { searchTerm: memberSearchTerm, page: 1, pageSize: 50 },
    ],
    queryFn: () =>
  ...
  const createMutation = useMutation({
    ...
    onError: (error: unknown) => {
      setError(
        extractErrorMessage(error, 'Failed to create training certificate')
      );
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: unknown) => {
      setError(
        extractErrorMessage(error, 'Failed to update training certificate')
      );
    },
  });

...
export interface CompleteReminderDrawerProps {
  open: boolean;
  onClose: () => void;
...
export function CompleteReminderDrawer({
  open,
  onClose,
  reminder,
  onSuccess,
}: CompleteReminderDrawerProps) {
    ...
    try {
      const response = await completeMutation.mutateAsync({
        ...
        request: {
          ...
          customDueDate:
            createNext && nextInterval === 'custom' && customDueDate
      ...
      setError(extractErrorMessage(err, 'Failed to complete reminder'));
    }
  };
        ...
        <Stack spacing={3} sx={{ mt: 3 }}>
          ...
          {createNext && (
            <>
              ...
              <Alert severity="info" icon={<InfoIcon />}>
                Next reminder will inherit description, assigned user, category,
                and priority from current reminder
              </Alert>
            </>
          )}

...
export class RegisterNumberService {
  private basePath = '/api/register-numbers';

  /**
   * Generate register numbers for a specific year
   */
  ...
  async previewRegisterNumbers(
    ...
    return apiClient.get<PreviewRegisterNumbersResponse>(
      `${this.basePath}/preview/${year}`
    );
  }

  /**
   * Check if register numbers have already been generated for a specific year
   */
  async getGenerationStatus(year: number): Promise<GenerationStatusResponse> {
    return apiClient.get<GenerationStatusResponse>(
      `${this.basePath}/status/${year}`
    );
  }

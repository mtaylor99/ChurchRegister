...
export class DistrictsApi {
  ...
  async getActiveDistrictOfficers(
    ...
    const params = excludeMemberId ? `?excludeMemberId=${excludeMemberId}` : '';
    return await apiClient.get<ChurchMemberSummary[]>(
  ...
  /**
   * Assign a description to a district
   * @param districtId The ID of the district
   * @param request The description assignment request
   */
  async assignDescription(
    districtId: number,
    request: AssignDescriptionRequest
  ): Promise<void> {
    return await apiClient.put<void>(
      `${this.basePath}/districts/${districtId}/assign-description`,
      request
    );
  }
  ...
  /**
   * Export districts members list as PDF
   * @returns PDF file as Blob
   */
  async exportDistrictsMemberList(): Promise<Blob> {
    const response = await fetch(
      `${apiClient.getBaseUrl()}${this.basePath}/districts/export-members-list`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${apiClient.getToken()}`,
    ...
    if (!response.ok) {
      throw new Error('Failed to export districts members list');
    }

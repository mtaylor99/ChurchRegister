...
describe('ContributionsApi', () => {
  ...
  describe('assignTransaction', () => {
    ...
    test('calls apiClient.post with secondary member for shared reference', async () => {
      const request = { churchMemberId: 5, secondaryChurchMemberId: 12 };
      await api.assignTransaction(10, request);
      expect(vi.mocked(apiClient.post)).toHaveBeenCalledWith(
        '/api/financial/hsbc-transactions/10/assign',
        request

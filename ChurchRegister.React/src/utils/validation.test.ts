  ...
  describe('validate', () => {
    test('should return valid result for passing validation', () => {
      const result = validate('test@example.com', [
        { fn: isValidEmail as (val: unknown) => boolean, message: 'Invalid email' },
      ]);
    ...
    test('should stop at first failing validation', () => {
      const result = validate('hi', [
        {
          fn: (value: unknown) => minLength(value as string, 5),
          message: 'Too short',
        },
        {
          fn: (value: unknown) => maxLength(value as string, 10),
          message: 'Too long',
        },
      ]);
    ...
    test('should pass all validations', () => {
      const result = validate('hello', [
        ...
        {
          fn: (value: unknown) => minLength(value as string, 3),
          message: 'Too short',
        },
        {
          fn: (value: unknown) => maxLength(value as string, 10),

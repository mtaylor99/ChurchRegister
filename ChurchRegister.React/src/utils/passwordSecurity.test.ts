...
describe('passwordSecurity', () => {
  describe('validatePassword', () => {
    ...
    test('should reject password that is too short', () => {
      ...
      expect(
        result.errors.some((e) => e.includes(`at least ${CHURCH_PASSWORD_POLICY.minLength}`))
      ).toBe(true);
    });
    ...
    test('should detect sequential characters', () => {
      ...
      expect(
        result.warnings?.some((w) => w.toLowerCase().includes('sequential'))
      ).toBe(true);
    });
    ...
    test('should detect repeating characters', () => {
      ...
      // but should have warnings
      expect(
        result.warnings?.some((w) => w.toLowerCase().includes('repeat'))
      ).toBe(true);
    });
    ...
    test('should detect keyboard patterns', () => {
      ...
      // but should have warnings
      expect(
        result.warnings?.some((w) => w.toLowerCase().includes('keyboard'))
      ).toBe(true);
    });
    ...
    test('should provide requirements status', () => {
      ...
      const lengthReq = result.requirements.find(
        (r) => r.category === 'length'
      );
  ...
  describe('isPasswordValid', () => {
    ...
    test('should return false for common password', () => {
      expect(isPasswordValid('iloveyou')).toBe(false);
    });
  ...
  describe('CHURCH_PASSWORD_POLICY', () => {
    ...
    test('should have defined special characters', () => {
      ...
      expect(CHURCH_PASSWORD_POLICY.specialCharacters.length).toBeGreaterThan(
        0
      );
    });
  });

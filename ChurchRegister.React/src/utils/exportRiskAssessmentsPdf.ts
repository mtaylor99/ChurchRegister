  ...
  try {
    ...
    assessments.forEach((assessment, index) => {
      ...
      const hasDescription =
        assessment.description && assessment.description.trim().length > 0;
      ...
      // Last Review
      pdf.text(
        formatDate(assessment.lastReviewDate),
        xPosition + 2,
        yPosition + 5
      );
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pdf.text(`Page ${i} of ${totalPages}`, pageWidth / 2, pageHeight - 7, {
        align: 'center',
      });
      pdf.text(`Generated: ${exportDate}`, pageWidth - margin, pageHeight - 7, {
        align: 'right',
      });
    }

...
export async function exportMemberContributionsToExcel(
  ...
  // Apply dark blue header row with white text
  const contribHeaderRange = XLSX.utils.decode_range(worksheet['!ref'] || 'A1');
  for (let col = contribHeaderRange.s.c; col <= contribHeaderRange.e.c; col++) {
...
export async function exportChurchMembersWithDetailsToExcel(
  ...
  memberNumbers?: Map<number, number | undefined>
): Promise<void> {
  // Dynamically import xlsx library to reduce initial bundle size
  const XLSX = await import('xlsx-js-style');

  // Sort members: those with a register number sorted by number ascending,
  // then those without sorted alphabetically by lastName then firstName.
  const withNumber = members
    .filter((m) => {
      const num = memberNumbers?.get(m.id);
      return num != null;
    })
    .sort((a, b) => {
      const na = memberNumbers!.get(a.id) ?? 0;
      const nb = memberNumbers!.get(b.id) ?? 0;
      return na - nb;
    });

  const withoutNumber = members
    .filter((m) => {
      const num = memberNumbers?.get(m.id);
      return num == null;
    })
  ...
  const sortedMembers = [...withNumber, ...withoutNumber];

  // Transform member data to Excel format - ordered to match grid columns
  const excelData = sortedMembers.map((member) => ({
    Title: member.title || '',
    ...
    'Gift Aid': member.giftAid ? 'Yes' : 'No',
    'Name in Communications': member.dataProtection?.allowNameInCommunications
      ? 'Yes'
      : 'No',
    'Health Status Mentions': member.dataProtection
      ?.allowHealthStatusInCommunications
      ? 'Yes'
      : 'No',
    'Photo in Print': member.dataProtection?.allowPhotoInCommunications
      ? 'Yes'
      : 'No',
    'Photo on Social Media': member.dataProtection?.allowPhotoInSocialMedia
      ? 'Yes'
      : 'No',
    'Group Photos': member.dataProtection?.groupPhotos ? 'Yes' : 'No',
    'Permission for Children': member.dataProtection?.permissionForMyChildren
      ? 'Yes'
      : 'No',
  ...
  // Apply dark blue header row with white text
  const membersHeaderRange = XLSX.utils.decode_range(worksheet['!ref'] || 'A1');
  for (let col = membersHeaderRange.s.c; col <= membersHeaderRange.e.c; col++) {
    const cellAddress = XLSX.utils.encode_cell({ r: 0, c: col });
    if (worksheet[cellAddress]) {
      worksheet[cellAddress].s = {
        font: { bold: true, color: { rgb: 'FFFFFF' } },
...
export async function exportTrainingCertificatesToExcel(
  ...
  // Apply dark blue header row with white text
  const certsHeaderRange = XLSX.utils.decode_range(worksheet['!ref'] || 'A1');
  for (let col = certsHeaderRange.s.c; col <= certsHeaderRange.e.c; col++) {
    const cellAddress = XLSX.utils.encode_cell({ r: 0, c: col });
    if (worksheet[cellAddress]) {
      worksheet[cellAddress].s = {
        font: { bold: true, color: { rgb: 'FFFFFF' } },

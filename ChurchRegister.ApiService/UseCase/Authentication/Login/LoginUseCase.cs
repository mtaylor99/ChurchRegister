...
public class LoginUseCase : ILoginUseCase
{
    ...
    private Task<string> GenerateTemporaryTokenAsync(ChurchRegisterWebUser user)
    {
        var key = Encoding.UTF8.GetBytes(_configuration["Jwt:Key"] ?? "ChurchRegister-Super-Secret-Key-For-Development-Only-2024!");
        
        // Use consistent UTC time
        var now = DateTime.UtcNow;
        var expires = now.AddMinutes(15); // 15 minute expiration for password change
        
        // Limited claims for temporary token - only password change permission
        var claims = new List<Claim>
        {
            new Claim(ClaimTypes.NameIdentifier, user.Id),
            new Claim(ClaimTypes.Email, user.Email!),
            new Claim("PasswordChangeOnly", "true"),
            new Claim(JwtRegisteredClaimNames.Iat, new DateTimeOffset(now).ToUnixTimeSeconds().ToString(), ClaimValueTypes.Integer64),
            new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString())
        };

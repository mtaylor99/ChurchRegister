...
public class UpdateProfileUseCase : IUpdateProfileUseCase
{
    ...
    public async Task<ChurchRegisterWebUser> ExecuteAsync(UpdateProfileRequest request, string userId, CancellationToken cancellationToken = default)
    {
        ...
        // Update the avatar based on name if not explicitly provided
        if (request.Avatar == null && !string.IsNullOrWhiteSpace(user.FirstName) && !string.IsNullOrWhiteSpace(user.LastName))
        {
            user.Avatar = $"{user.FirstName[0]}{user.LastName[0]}".ToUpper();
        }

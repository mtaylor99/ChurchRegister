    ...
    private void LogException(Exception exception, string correlationId, PathString path)
    {
        ...
        switch (exception)
        {
            case NotFoundException:
                // Resource not found - log as information
                _logger.LogInformation(exception, logMessage, correlationId, path, exception.GetType().Name);
                break;

            case ConflictException:
                // Expected business rule violation (e.g., resource at capacity) - log as debug only
                _logger.LogDebug(exception, logMessage, correlationId, path, exception.GetType().Name);
                break;

...
public class ChurchMemberService : IChurchMemberService
{
    ...
    public async Task<PagedResult<ChurchMemberDto>> GetChurchMembersAsync(ChurchMemberGridQuery query, CancellationToken cancellationToken = default)
    {
        ...
        // Apply no-bank-reference filter
        if (query.NoBankReferenceFilter == true)
        {
            membersQuery = membersQuery.Where(m => string.IsNullOrEmpty(m.BankReference));
        }
    ...
    {
        // Validate BankReference - allow up to 2 members (couples) to share the same reference
        if (!string.IsNullOrWhiteSpace(request.BankReference))
        {
            var membersWithSameReference = await _context.ChurchMembers
                .CountAsync(m => m.BankReference != null &&
                                m.BankReference.ToLower().Trim() == request.BankReference.ToLower().Trim(),
                            cancellationToken);

            if (membersWithSameReference >= 2)
                throw new ConflictException($"The bank reference '{request.BankReference}' is already shared by 2 members (maximum allowed). Please enter a different bank reference.");
        }
        ...
        try
        {
            ...
            if (member.ChurchMemberStatusId == 1)
            {
                try
                {
                    ...
                    if (request.MemberNumber.HasValue)
                    {
                        // Admin explicitly provided a number — use it directly
                        memberNumber = request.MemberNumber.Value;
    ...
    public async Task<ChurchMemberDetailDto> UpdateChurchMemberAsync(UpdateChurchMemberRequest request, string modifiedBy, CancellationToken cancellationToken = default)
    {
        ...
        member.Envelopes = request.Envelopes;
        ...
        if (member.ChurchMemberStatusId == 1 && request.MemberNumber.HasValue)
        {
            var currentYear = DateTime.UtcNow.Year;
            ...
            if (existingNumber != null)
            {
                ...
                existingNumber.Number = request.MemberNumber.Value;
            ...
            else
            {
                ...
                {
                    ChurchMemberId = member.Id,
                    Number = request.MemberNumber.Value,
                    Year = currentYear,
                    CreatedBy = modifiedBy,
                    CreatedDateTime = DateTime.UtcNow
                };

...
public class RegisterNumberService : IRegisterNumberService
{
        ...
        CancellationToken cancellationToken)
    {
        var existing = await _context.ChurchMemberRegisterNumbers
            ...
            .ThenBy(r => r.ChurchMember.LastName)
            .ToListAsync(cancellationToken);
        ...
        foreach (var record in existing)
        {
            ...
            var assignment = new RegisterNumberAssignment
            {
                ...
                MemberSince = record.ChurchMember.MemberSince ?? DateTime.UtcNow,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var baptisedMembers = activeMembers.Where(m => m.IsMember && m.Baptised).ToList();
        var nonBaptisedMembers = activeMembers.Where(m => m.IsMember && !m.Baptised).ToList();
        var nonMembers = activeMembers.Where(m => !m.IsMember).ToList();
